// 클레이튼 블록체인과 소통가능한 라이브러리 가져와서 비앱 안에서 쓸수있게 인스턴스화
...
const App = {
  ...
  handleLogin: async function () {
    if(this.auth.accessType == 'keystore'){
      try{
        // 키스토어와 비밀번호를 통해 프라이빗 키를 알수 있음 이를 통해 지갑을 만듬
  ...
  receiveKlay: function () {
    ...
    agContract.methods.transfer(cav.utils.toPeb("0.1", "KLAY")).send({
      ...
      if (receipt.status) {
        spinner.stop();
        alert("0,1 KLAY가 "+ walletInstance.address + " 계정으로 지급되었씁니다.");
        $('#transaction').html("");
