...
const App = {
    ...
    fileReader.onload = (event) => {
      try{
        if(!this.checkValidKeystore(event.target.result)){
          $('#message').text('not valid keystore file');
          return;
        }
        this.auth.keystore = event.target.result;
        $('#message').text('keystore pass!!');
  ...
  submitAnswer: async function () {
    ...
    if(answer === result ){
      if(confirm('success!!! 0.1KLY receive')){
  ...
  callContractBalance: async function () {
    return await agContract.methoeds.getBalance().call();
  },
      ...
      if(receipt.status){
        ...
        return agContract.methods.getBalance().call()
          .then(function(balance){
            ...
            $('#contractBalance').append('<p>' + '이벤트 잔액 : ' + 
              cav.utils.fromPeb(balance, 'KLAY'), + 'KLAY' + "</p>")
          });
      }
    })
  }
